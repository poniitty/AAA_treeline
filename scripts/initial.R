...
p <- st_read("/scratch/project_2007415/AAA_treelines/AAA_maxpoly3.gpkg") %>% 
  st_transform(4326)
...
p2 <- st_crop(p, gre %>% st_buffer(10))
p2 <- st_difference(p2, gre)
plot(st_geometry(p2))
plot(st_geometry(gre), add = TRUE, col = "red")
...
g <- st_make_grid(p, cellsize = 0.33333) %>% 
  st_as_sf()
sf_use_s2(FALSE)
g <- g[p,]
plot(g)
...
g %>% st_write("/scratch/project_2007415/AAA_treelines/AAA_treeline_initial_grid.gpkg", append = FALSE)
